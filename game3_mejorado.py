...
def verificar_palabra(word, secrets):
    ...
    for i, letter in enumerate(word):
        if letter == secrets[i]:
            feedback.append(f"👌 {letter}")  # Letra correcta en la posición correcta
        elif letter in secrets:
            feedback.append(f"🙄 {letter}")  # Letra correcta en posición incorrecta
        else:
            feedback.append(f"👎 {letter}")  # Letra incorrecta
